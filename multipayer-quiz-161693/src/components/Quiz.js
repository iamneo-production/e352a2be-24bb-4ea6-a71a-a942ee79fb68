...
displayQuestions = (questions = this.state.questions, currentQuestion, nextQuestion, previousQuestion) => {
    ...
    let { currentQuestionIndex } = this.state;   
        ...
        const answer = currentQuestion?.correctChoice;
        this.setState({
            currentQuestion,
            nextQuestion,
            previousQuestion,
...
render () {
    ...
    return (
            <div className="Quiz">
                ...
                <div>
                     <Typography display='inline' variant='h5'>{currentQuestion?.text}</Typography>
   ...
   useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        throw new Error(error.message || "Get trivia questions failed");
      }
    };

   fetchData().then((res)=>{setData(res)
                              console.log(Data)}); // Call the fetchData function when the component mounts
  }, []); // The empty dependency array ensures this effect runs once on mount

  const navigate = useNavigate();

      return(<Play data={Data} navigate ={navigate}></Play>)
